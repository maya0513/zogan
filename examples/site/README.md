...
The static introduction site explains zogan's Cache, Page, Fragment, and Island boundaries, its browser-owned navigation and form baseline, and its first setup. The English page at `/` is the default; the Japanese page is available at `/ja/`. It is intentionally separate from the Workers + D1 application in `examples/shop`.
